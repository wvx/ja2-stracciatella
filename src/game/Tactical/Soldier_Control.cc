...
{
	if (!s) return false;
	if (s->bWisdom<GCM->getGamePolicy()->all_skilltraits_wisdom) return false;
	if (s->bExpLevel<GCM->getGamePolicy()->all_skilltraits_level) return false;
	return true;
}
...
{
	if (!s) return false;
	if (s->bSide==ENEMY_TEAM && s->ubSoldierClass==SOLDIER_CLASS_ELITE && GCM->getGamePolicy()->enemy_elite_all_traits) return true;
	if (s->bSide!=OUR_TEAM && GCM->getGamePolicy()->all_skilltraits_just_ours) return false;
	if (s->bWisdom<GCM->getGamePolicy()->all_skilltraits_wisdom) return false;
	if (s->bExpLevel<GCM->getGamePolicy()->all_skilltraits_level) return false;
	return true;
}
...
UINT32 SoldierScroungeAmmoByGunItem(SOLDIERTYPE* const pSoldier, OBJECTTYPE* const weapon, UINT8 const prefered_ammo_type, UINT32 const enough, bool const full_access)
{
	...
	{
		if (GCM->getWeapon(weapon->usItem)->matches(mag) && mag->capacity == magazine_size)
		{
			ammo_type_item_index[mag->ammoType->index]=mag->getItemIndex();
		}
	}
...
bool SoldierAutoSwitchGoggles(SOLDIERTYPE* const pSoldier) //return true if changed
{
	...
	if(pSoldier->bSectorZ!=0)
	{
		...
		{
			if(SoldierFindFirstItemAndRemove(pSoldier, &item, NIGHTGOGGLES)) changed_faceitem=true;
		}
	}
	else
	{
		if(lowlight)
		{
			...
			{
				if(!SoldierFindFirstItemAndRemove(pSoldier, &item, UVGOGGLES))
				{
					if(SoldierFindFirstItemAndRemove(pSoldier, &item, NIGHTGOGGLES))
					changed_faceitem=true;
				}
				else changed_faceitem=true;
			}
		}
		else
		{
			if(IsWearingHeadGear(*pSoldier, SUNGOGGLES) && pSoldier->bSectorZ==0) return false;
			else
		{
				if(SoldierFindFirstItemAndRemove(pSoldier, &item, SUNGOGGLES)) changed_faceitem=true;
			}
		}
	}
	...
	{
			if(SoldierFindFirstItemAndRemove(pSoldier, &item, GASMASK))
			{
				SoldierUnequipFaceItems(pSoldier);
				SoldierItemAutoPlaceThenDrop(pSoldier, &item, false);
				changed_faceitem=true;
...
void SoldierEquipFromLoadedSector(SOLDIERTYPE* const pSoldier, bool const full_access)
{
	...
	UINT16 aEquipmentList[] =		{	MORTAR,GLOCK_17,ROCKET_LAUNCHER,COMBAT_KNIFE,THROWING_KNIFE,
																SNIPERSCOPE,LASERSCOPE,BIPOD,GUN_BARREL_EXTENDER,SILENCER,UNDER_GLAUNCHER,
																FLAK_JACKET,STEEL_HELMET,KEVLAR_LEGGINGS,CERAMIC_PLATES,
																EXTENDEDEAR,NIGHTGOGGLES,SUNGOGGLES,GASMASK};

	UINT16 aEquipmentExtras[] =	{	HAND_GRENADE,TEARGAS_GRENADE,MUSTARD_GRENADE,STUN_GRENADE,
																CAMOUFLAGEKIT,BREAK_LIGHT,FIRSTAIDKIT,CANTEEN,
																MINI_GRENADE,MINI_GRENADE,BRASS_KNUCKLES,CROWBAR};

	UINT16 aEquipmentListEnemyExtras[]	=	{	CIGARS,GOLDWATCH,__ITEM_257,CHEWING_GUM,MEDICKIT,
		...
		if(pSoldier->inv[i].usItem!=NOTHING)
		{
			...
			switch(GCM->getItem(pSoldier->inv[i].usItem)->getItemClass())
			{
				...
				case IC_GUN:
				{
					...
					//TODO attach 40mm ammo to GL attachment
					break;
				}
	...
	if(pSoldier->bTeam!=OUR_TEAM)
	{
		pSoldier->bCamo = (FindObj(pSoldier, CAMOUFLAGEKIT) != NO_SLOT) ? 100 : 0; //0 they put it back in the tube...
	}
...
{
	if(!GCM->getGamePolicy()->militia_use_sector_inventory) return;
