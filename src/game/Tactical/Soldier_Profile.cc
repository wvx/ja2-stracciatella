...
void UnRecruitMilitiaSoldier( ProfileID const pid )
{
	...
	if (pMilitiaExisting)
	{
		...
		{
			if (pMilitiaExisting->bLife > 0) pSector->ubNumberOfCivsAtLevel[GREEN_MILITIA]++;
			pNewSoldier->ubSoldierClass=SOLDIER_CLASS_GREEN_MILITIA;
		}
		if (gMercProfiles[pid].zNickname[0] == L'R')
		{
			if (pMilitiaExisting->bLife > 0) pSector->ubNumberOfCivsAtLevel[REGULAR_MILITIA]++;
			pNewSoldier->ubSoldierClass=SOLDIER_CLASS_REG_MILITIA;
		}
		if (gMercProfiles[pid].zNickname[0] == L'V')
		{
			if (pMilitiaExisting->bLife > 0) pSector->ubNumberOfCivsAtLevel[ELITE_MILITIA]++;
